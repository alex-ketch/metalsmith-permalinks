  ...
  return function(files, metalsmith, done){
    ...
    Object.keys(files).forEach(function(file){
      ...
      if (linkset.relative) {
        ...
        if (linkset.flatten) {
          var _tempPath = file.split('/');
          var parentPath = _tempPath.slice(0, _tempPath.length - 1).join('/');

          Object.keys(files).forEach(function(file){
            var match = parentPath && file.indexOf(parentPath) >= 0;
            if (match) {
              delete files[file];
            }
          })
...
function normalize(options){
  ...
  options.flatten  = options.hasOwnProperty('flatten') ? options.flatten : false;
